...
// =================================================================================
// 🔥 ADIM 3: DİKKAT! BU ALANI KENDİ FIREBASE ANAHTARLARINIZLA GÜNCELLEYİN!
// =================================================================================

// 1. Kendi Firebase projenizden aldığınız yapılandırmayı buraya ekleyin.
// Örnek: projectId: "dnd-proje-adi", apiKey: "AIzaSy_YOUR_API_KEY", ...
...
// 2. Uygulama ID'si (Veritabanı yollarında kullanmak için Proje ID'sini kullanıyoruz)
...
// Canvas Ortamından Gelen Değişkenler (Dışarıda kullanılamayacağı için yedekleniyor)
...
if (Object.keys(firebaseConfig).length > 0 && firebaseConfig.projectId !== 'YOUR_PROJECT_ID_HERE') {
  ...
  auth = getAuth(firebaseApp);
    ...
    // Proje ID'si güncellenmemişse hata mesajı ver
    console.error("🔥🔥🔥 LÜTFEN EXTERNAL_FIREBASE_CONFIG'İ KENDİ ANAHTARLARINIZLA GÜNCELLEYİN! 🔥🔥🔥");
}
...
const useFirebase = () => {
  ...
  useEffect(() => {
    if (!auth) {
        setIsAuthReady(true); // Auth objesi yoksa (hata durumunda) hemen hazır say
        return;
    }
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        // Canvas'a özgü kimlik doğrulama veya anonim giriş
        try {
          if (initialAuthToken) {
            // SADECE CANVAS ORTAMINDA ÇALIŞIR
            await signInWithCustomToken(auth, initialAuthToken);
          } else {
            // GITHUB/HARİCİ ORTAMDA VEYA TOKEN YOKSA ANONİM GİRİŞ
...
const rollDice = (diceString) => {
  ...
  if (!match) return `Hata: Geçersiz zar formatı (${diceString}). Örnek: 1d20+5`;

  const numDice = parseInt(match[1] || '1', 10);
  const dieSize = parseInt(match[2], 10);
  const modifier = parseInt(match[3] || '0', 10);

  if (numDice <= 0 || dieSize <= 0) return "Hata: Zar sayısı veya boyutu sıfırdan büyük olmalı.";
  ...
  return {
    diceString,
    result,
    details: `${numDice}d${dieSize}${match[3] || ''} atışı: ${totalRoll}${modifier !== 0 ? (modifier > 0 ? ` + ${modifier}` : ` - ${Math.abs(modifier)}`) : ''} = ${result}${rollDetails}`
  };
};
...
const DiceRoller = ({ onRoll, defaultDice = '1d20' }) => {
  ...
  return (
    <div className="p-4 bg-gray-100 rounded-xl shadow-inner mt-4">
      <h3 className="text-lg font-bold mb-3 text-gray-700">Kaos Zarları</h3>
      <div className="flex space-x-2">
        <input
          type="text"
          value={diceInput}
          onChange={(e) => setDiceInput(e.target.value)}
          placeholder="Örn: 1d20+5"
      ...
      {lastResult && (
        <div className="mt-3 p-3 bg-white border border-gray-200 rounded-lg text-sm">
          <p className="font-semibold text-xl text-indigo-700">Sonuç: {lastResult.result}</p>
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-900 text-white p-4">
      <div className="bg-gray-800 p-10 rounded-2xl shadow-2xl w-full max-w-md">
        ...
        <p className="mb-6 text-gray-300 text-center">Maceraya başlamadan önce ismini gir!</p>
        <input
          type="text"
          value={inputName}
...
const MainMenu = ({ setCurrentView, setJoinCode, userName }) => {
  ...
  return (
    <div className="flex flex-col items-center min-h-screen bg-gray-100 p-8">
      <h1 className="text-3xl font-bold mb-8 text-indigo-700">Hoş Geldin, {userName}!</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-4xl">
        ...
        <div className="bg-white p-6 rounded-xl shadow-lg hover:shadow-xl transition duration-300">
          ...
          <p className="text-gray-600 mb-4">Yeni bir maceraperest oluştur ve tüm karakterlerini yönet.</p>
          <Button onClick={() => setCurrentView('create-char')} color="blue" className="w-full">
            Karakter Yarat / Yönet
          </Button>
        </div>

        {/* Masa Aç */}
        <div className="bg-white p-6 rounded-xl shadow-lg hover:shadow-xl transition duration-300">
          <h2 className="text-xl font-semibold mb-3 text-gray-800">Masa Aç (DM)</h2>
          <p className="text-gray-600 mb-4">Yeni bir oyun oturumu başlat ve Dungeon Master ol.</p>
        ...
        <div className="bg-white p-6 rounded-xl shadow-lg hover:shadow-xl transition duration-300">
          ...
          <p className="text-gray-600 mb-4">DM'den aldığın kod ile mevcut bir oyuna katıl.</p>
          <input
            type="text"
            value={code}
...
const CharacterCreator = ({ setCurrentView, userId, db }) => {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Bu karakteri silmek istediğine emin misin?")) {
      try {
  ...
  const handlePdfDownload = (char) => {
    ...
    printWindow.document.write('<html><head><title>Karakter Kağıdı</title>');
    ...
    return (
      ...
      >
        <div className="flex justify-between items-start mb-4">
          <h2 className="text-2xl font-bold text-gray-800">{char.name || 'İsimsiz Karakter'}</h2>
          <div className="flex space-x-2 no-print">
            {allowEdit && (
              <Button onClick={() => handleEdit(char)} color="gray" className="py-1 px-3 text-sm">Düzenle</Button>
            )}
            <Button onClick={() => handlePdfDownload(char)} color="gray" className="py-1 px-3 text-sm">PDF İndir</Button>
            {allowEdit && (
        ...
        <div className="text-gray-600 mb-4">
          ...
          <p><strong>Sınıf:</strong> {char.class} ({char.level}. Seviye)</p>
        </div>
  ...
  return (
    <div className="p-8 bg-gray-100 min-h-screen">
      <div className="max-w-6xl mx-auto">
        ...
        <h1 className="text-4xl font-extrabold text-indigo-700 mb-8">Karakter Yaratma ve Yönetim</h1>

        {/* Character Creation Form */}
        <div className="bg-white p-8 rounded-xl shadow-xl mb-10">
          <h2 className="text-2xl font-bold mb-6 text-gray-800">{editingId ? 'Karakteri Düzenle' : 'Yeni Karakter Oluştur'}</h2>
          <form onSubmit={handleSave} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <input type="text" name="name" value={newChar.name} onChange={handleChange} placeholder="Karakter Adı" required className="p-3 border rounded-lg focus:ring-indigo-500" />
              ...
              <input type="text" name="class" value={newChar.class} onChange={handleChange} placeholder="Sınıf (Class)" className="p-3 border rounded-lg focus:ring-indigo-500" />
            </div>
            ...
            <textarea name="notes" value={newChar.notes} onChange={handleChange} placeholder="Önemli Notlar, Envanter, Büyüler..." rows="5" className="p-3 border rounded-lg w-full focus:ring-indigo-500"></textarea>
            <Button type="submit" color="blue" className="w-full">
              {editingId ? 'Karakteri Güncelle' : 'Karakteri Kaydet'}
            </Button>
            {editingId && (
        ...
        {/* Character List */}
        <h2 className="text-3xl font-bold text-gray-700 mb-6">Yarattığın Karakterler ({characters.length})</h2>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {characters.length === 0 ? (
            <p className="text-gray-500 lg:col-span-2">Henüz karakter oluşturmadın. Yukarıdaki formu kullanarak bir tane oluştur.</p>
          ) : (
            characters.map(char => (
...
const GameTable = ({ setCurrentView, activeTableId, isDM, userName, userId, db, characters, setJoinCode }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribeTable = onSnapshot(doc(db, tableDocPath), (docSnapshot) => {
      if (docSnapshot.exists()) {
        ...
        setTable(null);
        console.error("Hata: Masa bulunamadı veya silinmiş.");
        // alert("Hata: Masa bulunamadı veya silinmiş."); // Alert yerine console.error kullanıldı
        setCurrentView('menu');
        setJoinCode('');
      }
    }, (error) => {
      console.error("Masa verisi çekilirken hata:", error);
    });
    ...
    const unsubscribeChat = onSnapshot(query(collection(db, chatCollectionPath)), (snapshot) => {
      ...
      console.error("Chat mesajları çekilirken hata:", error);
    });
  ...
  const handleDiceRoll = async (details, result) => {
    try {
      ...
      console.error("Zar atışı kaydedilemedi:", error);
    }
  };
  ...
  const handleSelectCharacter = async (charId) => {
    try {
      ...
      const charName = characters.find(c => c.id === charId)?.name || "Seçilen Karakter";
      await addDoc(collection(db, chatCollectionPath), {
        text: `${userName} masaya "${charName}" karakteriyle katıldı.`,
        senderName: 'Sistem',
        senderId: 'system',
        timestamp: serverTimestamp(),
        type: 'system',
      ...
      console.error("Karakter seçilemedi:", error);
    }
  };
  ...
  const handleDMSaveEdit = async (updatedChar) => {
    ...
    try {
      ...
      console.error("DM karakteri güncellerken hata:", error);
    }
  };
  ...
  if (!table) return <div className="p-8 text-center text-xl text-gray-500">Masa yükleniyor...</div>;
    ...
    <div className="flex h-screen bg-gray-900">
      ...
      <div className="w-1/4 bg-gray-800 p-6 flex flex-col space-y-6 overflow-y-auto">
        ...
        <div className="bg-gray-700 p-4 rounded-xl">
          <h3 className="text-lg font-semibold text-gray-200 mb-2">Karakterini Seç</h3>
          <select
          ...
          {!myChar && selectedCharId && <p className="text-red-300 text-sm mt-2">Karakterin DM tarafından onaylanmalı.</p>}
        </div>
              ...
              return (
                <li key={player.userId} className={`p-3 rounded-lg flex justify-between items-center ${player.userId === userId ? 'bg-indigo-600' : 'bg-gray-700'}`}>
                  <div className="text-sm">
                    ...
                    <p className="text-xs italic text-gray-300">{char ? char.name : 'Karakter Seçilmedi'}</p>
                  </div>
                  {isDM && player.charId && (
      ...
      <div className="flex-grow flex flex-col p-6 space-y-6 overflow-y-auto">
        ...
        <div className="bg-gray-800 p-6 rounded-xl shadow-lg flex-shrink-0">
          <h3 className="text-xl font-bold text-white mb-4">{isDM ? 'DM Düzenleme Alanı' : 'Karakter Kağıdınız'}</h3>
          ...
          {isDM && currentEditChar ? (
            ...
            myChar ? (
              ...
              <p className="text-gray-400">Masaya katılmak için soldan bir karakter seçin.</p>
            )
          )}
        </div>
        ...
        <div className="flex-grow bg-gray-800 p-4 rounded-xl shadow-lg flex flex-col">
          ...
          <form onSubmit={handleSendMessage} className="flex space-x-2 pt-3 border-t border-gray-700">
            <input
              ...
              required
            />
            <Button type="submit" color="blue">Gönder</Button>
          </form>
        </div>
      </div>
    </div>
...
const PlayerCharSheet = ({ char, onPdfDownload }) => {
  ...
  return (
    <div id="player-char-sheet" className="bg-white p-6 rounded-xl text-gray-800">
      <div className="flex justify-between items-start mb-4">
        <h2 className="text-2xl font-bold">{char.name || 'İsimsiz Karakter'}</h2>
        <Button onClick={onPdfDownload} color="gray" className="py-1 px-3 text-sm">PDF İndir</Button>
      </div>

      <div className="text-gray-600 mb-4">
        <p><strong>Irk:</strong> {char.race} | <strong>Sınıf:</strong> {char.class} ({char.level}. Seviye)</p>
      </div>
...
const DMEnhancedCharEditor = ({ char, onSave, onCancel }) => {
  ...
  const handlePdfDownload = () => {
    ...
    const printWindow = window.open('', '', 'height=600,width=800');
    printWindow.document.write('<html><head><title>DM Düzenleme Kağıdı</title>');
  ...
  return (
    <div ref={charSheetRef} className="bg-white p-6 rounded-xl shadow-lg border-t-4 border-red-500 text-gray-800">
      <h3 className="text-2xl font-bold mb-4">DM: {editedChar.name} Karakterini Düzenle</h3>
      <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); onSave(editedChar); }}>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <input type="text" name="name" value={editedChar.name} onChange={handleChange} placeholder="Karakter Adı" required className="p-3 border rounded-lg focus:ring-red-500" />
          ...
          <input type="text" name="class" value={editedChar.class} onChange={handleChange} placeholder="Sınıf (Class)" className="p-3 border rounded-lg focus:ring-red-500" />
        </div>
        ...
        <textarea name="notes" value={editedChar.notes} onChange={handleChange} placeholder="Önemli Notlar, Envanter, Büyüler..." rows="5" className="p-3 border rounded-lg w-full focus:ring-red-500"></textarea>
        
        <div className="flex space-x-4">
          <Button type="submit" color="red" className="flex-grow">Değişiklikleri Kaydet</Button>
          <Button type="button" onClick={onCancel} color="gray" className="flex-grow">İptal Et</Button>
          <Button type="button" onClick={handlePdfDownload} color="gray" className="flex-grow">PDF İndir</Button>
        </div>
      </form>
    </div>
  );
...
const CreateGameLogic = ({ setCurrentView, setActiveTableId, userName, userId, db }) => {
  ...
  const createGame = async () => {
    ...
    // Auth objesinin varlığını kontrol et
    if (!db) {
        setError("Firebase bağlantısı yok. Lütfen yapılandırma anahtarlarını kontrol edin.");
        setLoading(false);
        return;
    }
    ...
    try {
      ...
      await addDoc(collection(db, `${tableDocPath}/chat`), {
        text: `${userName} (DM) masayı kurdu! Oyuncuların kodu girerek katılması bekleniyor.`,
        senderName: 'Sistem',
        senderId: 'system',
        timestamp: serverTimestamp(),
        type: 'system',
      ...
      console.error("Masa oluşturulurken hata:", e);
      setError("Masa oluşturulurken bir hata oluştu. Lütfen tekrar deneyin. Firestore kurallarını kontrol edin.");
    } finally {
      setLoading(false);
    }
  };
...
const JoinGameLogic = ({ setCurrentView, joinCode, setActiveTableId, userName, userId, db, characters, setJoinCode }) => {
  ...
  const joinGame = async () => {
    ...
    try {
      ...
      if (!docSnap.exists()) {
        setError(`Hata: ${joinCode} kodlu masa bulunamadı.`);
      } else {
        ...
        if (!existingPlayer) {
          ...
          // Send a system message to chat
          const messageText = `${userName} masaya katıldı!`;
          await addDoc(collection(db, `${tableDocPath}/chat`), {
            text: messageText,
            senderName: 'Sistem',
            senderId: 'system',
      ...
      console.error("Masaya katılırken hata:", e);
      setError("Masaya katılırken beklenmedik bir hata oluştu. Firestore kurallarını kontrol edin.");
    } finally {
      setLoading(false);
    }
  };
        ...
        return (
             <div className="flex justify-center items-center min-h-screen text-2xl text-red-700 bg-red-100 p-8">
                <div className='p-6 bg-white rounded-xl shadow-lg'>
                    <p className='font-bold mb-4'>HATA: Firebase Yapılandırması Eksik</p>
                    <p className='text-base text-gray-700'>Lütfen <code>DnD_Lobby.jsx</code> dosyasının tepesindeki <code>EXTERNAL_FIREBASE_CONFIG</code> alanını kendi Firebase projenizin anahtarlarıyla güncelleyin.</p>
                </div>
            </div>
        );
    }
